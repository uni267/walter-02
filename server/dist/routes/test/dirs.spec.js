...
describe(dir_url, function () {
  ...
  describe("get /", function () {
    ...
    var root_id;
    var child_id; // 親のid, 1階層子のidを取得する

    before(function (done) {
      new Promise(function (resolve, reject) {
        request.post(dir_url).send({
          dir_id: user.tenant.home_dir_id,
          dir_name: "create dir ok"
        }).end(function (err, res) {
          return resolve(res);
        });
      }).then(function (res) {
        return new Promise(function (resolve, reject) {
          request.get(dir_url).query({
            dir_id: user.tenant.home_dir_id
          }).end(function (err, res) {
            root_id = res.body.body[0]._id;
            request.get(dir_url + "/tree").query({
              root_id: root_id
            }).end(function (err, res) {
