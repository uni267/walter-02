...
var tenant_id = "";
var file_id;
var login_user; // テスト用のアップロードファイル(client側から送信しているPayload)
...
describe(base_url, function () {
  before(function (done) {
    _builder.initdbPromise.then(function () {
      request.post(login_url).send(_builder.authData).end(function (err, res) {
        ...
        request.set('x-auth-cloud-storage', res.body.body.token); // テスト用のファイルをアップロード
  ...
  describe('post /files', function () {
    var response;
    var url = "".concat(base_url, "/files");
    before(function (done) {
      // 権限一覧を取得
      request.get('/api/v1/role_files').end(function (err, res) {
        (0, _chai.expect)(res.status).equal(200); // ファイル権限を追加

        request.post("/api/v1/files/".concat(file_id, "/authorities")).send({
          user: login_user,
    ...
    describe('正常系', function () {
      describe('存在しないfile_idを指定', function () {
        before(function (done) {
          var body = {
            files: [new _mongoose["default"].Types.ObjectId()]
          };
          request.post("".concat(base_url, "/files")).send(body).end(function (err, res) {
            response = res;
            done();
          });
        });
