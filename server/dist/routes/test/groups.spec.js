...
describe(groups_url + "/:group_id", function () {
    ...
    describe("name, descriptionを指定し正しく作成した場合", function () {
      ...
      describe("作成したグループを取得した場合", function () {
        ...
        before(function (done) {
          ...
          request.get(groups_url + "/".concat(group_id)).end(function (err, res) {
            nextPayload = res;
            done();
          });
        });
  ...
  describe("get /:group_id", function () {
    ...
    before(function (done) {
      new Promise(function (resolve, reject) {
        ...
        return new Promise(function (resolve, reject) {
          request.get(groups_url + "/".concat(group._id)).end(function (err, res) {
            return resolve(res);
          });
        });
      }).then(function (res) {
  ...
  describe("delete /:group_id", function () {
    ...
    describe("存在するgroup_idを指定した場合", function () {
      var payload;
      before(function (done) {
        request["delete"](groups_url + "/".concat(group._id)).end(function (err, res) {
          payload = res;
          done();
        });
      });
  ...
  describe("patch /:group_id/name", function () {
    ...
    describe("正しいgroup_id, nameを指定した場合", function () {
      ...
      before(function (done) {
        request.patch(groups_url + "/".concat(group._id, "/name")).send(body).end(function (err, res) {
          payload = res;
          done();
        });
      });
  ...
  describe("patch /:group_id/description", function () {
    ...
    describe("正しいgroup_id, descriptionを指定した場合", function () {
      ...
      before(function (done) {
        request.patch(groups_url + "/".concat(group._id, "/description")).send(body).end(function (err, res) {
          payload = res;
          done();
        });
      });
    ...
    describe("descriptionが255文字を超過する場合", function () {
      var payload;
      var body = {
        description: _.range(257).map(function (i) {
          return "1";
        }).join("")
      };
      var expected = {
  ...
  describe("patch /:group_id/role_menus", function () {
    ...
    describe("正しいgroup_id, role_menu_idを指定した場合", function () {
      var payload;
      before(function (done) {
        request.patch(groups_url + "/".concat(group_id, "/role_menus")).send({
    ...
    describe("group_idがoid形式ではない場合", function () {
      ...
      before(function (done) {
        request.patch(groups_url + "/invalid_oid/role_menus").send({
          role_menu_id: role_menu_id
        }).end(function (err, res) {
          payload = res;
          done();
    ...
    describe("role_menu_idがoid形式ではない場合", function () {
      ...
      before(function (done) {
        request.patch(groups_url + "/".concat(group_id, "/role_menus")).send(body).end(function (err, res) {
          payload = res;
          done();
        });
      });
