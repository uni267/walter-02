...
describe(base_url, function () {
  ...
  describe('get /', function () {
    ...
    it('_idはmongoose.Types.ObjectIdである', function (done) {
      var tag = (0, _lodash.first)(response.body.body);
      (0, _chai.expect)(_mongoose["default"].Types.ObjectId.isValid(tag._id)).equal(true);
      done();
    });
  ...
  describe('get /:tag_id', function () {
    describe('異常系', function () {
      ...
      describe('存在しないtag_idを指定', function (done) {
        ...
        var response;
        before(function (done) {
          var newId = new _mongoose["default"].Types.ObjectId();
          request.get("".concat(base_url, "/").concat(newId)).end(function (err, res) {
            response = res;
            done();
          });
        });
    ...
    describe('正常系', function () {
      ...
      before(function (done) {
        request.get(base_url).end(function (err, res) {
          var id = (0, _lodash.first)(res.body.body)._id; // 先頭のtagを対象とする
      ...
      it('labelはstringである', function (done) {
        (0, _chai.expect)((0, _typeof2["default"])(response.body.body.label)).equal('string');
        done();
      });
      it('tagsにはtenant_idが含まれる', function (done) {
  ...
  describe('delete /:tag_id', function () {
    ...
    describe('異常系', function () {
      describe('tag_idが未定義', function () {
        it('http(404)が返却される', function (done) {
          request["delete"]("".concat(base_url, "/")).end(function (err, res) {
      ...
      describe('存在しないtag_idを指定', function () {
        ...
        before(function (done) {
          var newId = new _mongoose["default"].Types.ObjectId();
          request["delete"]("".concat(base_url, "/").concat(newId)).end(function (err, res) {
            response = res;
            done();
          });
        });
    ...
    describe('正常系', function () {
      var response = {};
      var DataLengthBeforDelete; // 削除前データ数
  ...
  describe('patch /:tag_id/label', function () {
    ...
    describe('異常系', function () {
      describe('存在しないtag_idを指定', function () {
        ...
        before(function (done) {
          var newId = new _mongoose["default"].Types.ObjectId();
          request.patch("".concat(base_url, "/").concat(newId, "/label")).send(body).end(function (err, res) {
            response = res;
            done();
          });
        });
      ...
      describe('labelが未定義', function () {
        ...
        describe('labelがundefined', function () {
          ...
          var response;
          before(function (done) {
            request.patch("".concat(base_url, "/").concat(createdData._id, "/label")).send(body).end(function (err, res) {
              response = res;
              done();
            });
          });
    ...
    describe('正常系', function () {
      describe('labelのみを送信', function () {
        ...
        it('送信したlabelに変更されること', function (done) {
          request.get("".concat(base_url, "/").concat(createdData._id)).end(function (err, res) {
  ...
  describe('patch /:tag_id/color', function () {
    ...
    describe('異常系', function () {
      describe('存在しないtag_idを指定', function () {
        ...
        before(function (done) {
          var newId = new _mongoose["default"].Types.ObjectId();
          request.patch("".concat(base_url, "/").concat(newId, "/color")).send(body).end(function (err, res) {
            response = res;
            done();
          });
        });
      ...
      describe('colorに16進トリプレット表記以外を指定', function () {
        ...
        var response = {};
        before(function (done) {
          request.patch("".concat(base_url, "/").concat(createdData._id, "/color")).send(body).end(function (err, res) {
            response = res;
            done();
          });
        });
